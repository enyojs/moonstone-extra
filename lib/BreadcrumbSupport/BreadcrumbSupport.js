...
exports.Support = {
	...
	*/
	...
	updatePanelWidth: function() {
		var bounds = this.getBounds(), panels = this.getPanels();
		if (panels.length > 0 && bounds.width != undefined && !panels[0]._resized) {
			panels[0].applyStyle('width', bounds.width + 'px');
			panels[0].resize();
			panels[0]._resized = true;
		}
	},
