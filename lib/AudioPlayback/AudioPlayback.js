...
module.exports = kind(
	...
	*/
	mediaComponents: [
		...
		{name: 'sliderContainer', classes: 'moon-audio-playback-bottom', components: [
			{name: 'slider', kind: MediaSlider, rtl: false,
				// onSeekStart: 'sliderSeekStart',
				onSeek: 'sliderChanging',
				// onSeekFinish: 'sliderSeekFinish',
				onEnterTapArea: 'enterTapArea',
				onLeaveTapArea: 'leaveTapArea',
		...
		// {rtl: false, classes: 'moon-audio-playback-bottom', components: [
			...
			// rtl: false, noPopup: true, lockBar: true,
			// 	onChanging: 'sliderChanging',
			// 	onAnimateFinish: 'sliderChanging',
			// 	onmove: 'preview',
	...
	*/
	updatePlayTime: function (start, end) {
		this.$.slider.set('max', end);
		this.$.slider.setValue(start);
		console.log("updatePlayTime:", start, end);
	...
	*/
	sliderChanging: function (sender, ev) {
		...
		var totalTime = this.getDuration(),
			currentTime = (totalTime / 100) * ev.value;
		// this.updatePlayTime(currentTime, totalTime);
		// this.updatePlayTime(this.toReadableTime(currentTime), this.toReadableTime(totalTime));
	...
	*/
	commandHandler: function (sender, ev) {
		...
		this[command] && this[command](sender, ev);
		return true;
	},
