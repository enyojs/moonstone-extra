...
* control.
*
* @typedef {Object} moon.MediaSlider~FeedbackParameterObject
...
*
* @event moon.MediaSlider#onSeekStart
* @type {Object}
* @public
*/
...
*
* @event moon.MediaSlider#onSeekFinish
* @type {Object}
* @property {Number} value - The position to seek to.
* @public
*/
...
*
* @event moon.MediaSlider#onEnterTapArea
* @type {Object}
* @public
*/
...
*
* @event moon.MediaSlider#onLeaveTapArea
* @type {Object}
* @public
*/

/**
* {@link moon.MediaSlider} extends {@link moon.Slider}, adding specialized
* behavior related to video playback.
*
* ```javascript
* {kind: 'MediaSlider, value: 30}
* ```
*
* The [onSeekStart]{@link moon.MediaSlider#onSeekStart} event is fired while
* the control knob is being dragged, the
* [onSeekFinish]{@link moon.MediaSlider#onSeekFinish} event is fired when the
* drag finishes, and the [onSeek]{@link moon.MediaSlider#onSeek} event is fired
* when the position is set by tapping the bar.
*
* @class moon.MediaSlider
* @extends moon.Slider
* @ui
* @public
*/
module.exports = kind(
	/** @lends moon.MediaSlider.prototype */ {

	/**
	* @private
	*/
	name: 'moon.MediaSlider',
	...
	*/
	published: {
		...
		*/
		...
		/**
		* Ending time of media (typically this will be the duration).
		*
		* @type {Number}
		* @default 0
		* @public
	...
	*/
	updateSliderRange: function () {
		...
		console.log('updateSliderRange:', this.min, this.max);
	...
	*/
	rangeStartChanged: function () {
		this.rangeStart = this.clampValue(this.getMin(), this.getMax(), this.rangeStart);
		this.updateInternalProperty();
	...
	*/
	rangeEndChanged: function () {
		this.rangeEnd = this.clampValue(this.getMin(), this.getMax(), this.rangeEnd);
		this.updateInternalProperty();
	},
	...
	*/
	...
	updateScale: function () {
		var factor = (this.rangeEnd-this.rangeStart)/(this.max-this.min);
		this.scaleFactor = isNaN(factor) ? 1 : factor;
	},
	...
	*/
	...
	// 	console.log('calcPercent:', val, this.scaleFactor);
	...
	*/
	_calcPercent: function (val) {
		console.log('_calcPercent:', val);
		// return this.calcRatio(val);
	...
	*/
	updatePopupPosition: function (val) {
		console.log('updatePopupPosition:', !this.dragging, this.isInPreview());
	...
	*/
	_updateKnobPosition: function (val) {
		// console.log('_updateKnobPosition1:', this.min, this.max, val);
		// Setting the knob position 1 tick before the last frame allows the video frame to update before shutting off.
		var p = this.clampValue(this.min, (this.max ? this.max : 0) , val);
		// console.log('_updateKnobPosition2:', this.min, this.max, val, p);
		p = this._calcPercent(p);
		if (isNaN(p)) { p = 0; }
		// console.log('_updateKnobPosition3:', this.min, this.max, val, p);
		var slider = this.inverseToSlider(p);
		console.log('knob left:', slider, val, p);
	...
	*/
	inverseToSlider: function (percent) {
		console.log('inverseToSlider:', percent, this._calcPercent(this.rangeStart), this.rangeStart, this.scaleFactor);
	...
	*/
	spotFocused: function (sender, e) {
		...
		if (!this.disabled) {
			...
			//fires enyo.MediaSlider#onEnterTapArea
			this.doEnterTapArea();
		}
	},
	...
	*/
	spotBlur: function () {
		...
		this.endPreview();
		//fires enyo.MediaSlider#onLeaveTapArea
		this.doLeaveTapArea();
	...
	*/
	...
	/**
	* If `dragstart`, bubbles [onSeekStart]{@link moon.MediaSlider#onSeekStart}
	* event.
	*
	* @fires moon.MediaSlider#onSeekStart
	* @private
	*/
	...
	/**
	* If `drag`, bubbles [onSeek]{@link moon.MediaSlider#onSeek} event and
	...
	* [onSeekFinish]{@link moon.MediaSlider#onSeekFinish} event and overrides
	* parent `dragfinish` handler.
	*
	* @fires moon.MediaSlider#onSeekFinish
	* @private
	*/
	...
	/**
	* Sends [onSeek]{@link moon.MediaSlider#onSeek} event.
	*
	* @fires moon.MediaSlider#onSeek
	* @private
	*/
	...
	* @param {moon.MediaSlider~FeedbackParameterObject} params - A
