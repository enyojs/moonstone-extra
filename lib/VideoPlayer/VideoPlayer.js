...
var
	...
	MediaSlider = require('../MediaSlider'),
...
module.exports = kind(
			...
			{name: 'playerControl', kind: Control, classes: 'moon-video-player-bottom', showing: false, components: [
				{name: 'controls', kind: FittableColumns, rtl:false, classes: 'moon-video-player-controls-frame', ontap: 'resetAutoTimeout', components: [
					...
					{name: 'controlsContainer', kind: Panels, arrangerKind: CarouselArranger, fit: true, draggable: false, classes: 'moon-video-player-controls-container', components: [
						{name: 'trickPlay', kind: Control, ontap:'playbackControlsTapped', components: [
							{name: 'playbackControls', kind: Control, classes: 'moon-video-player-control-buttons', components: [
								{name: 'jumpBack',		kind: IconButton, small: false, backgroundOpacity: 'translucent', onholdpulse: 'onHoldPulseBackHandler', ontap: 'onjumpBackward', onrelease: 'onReleaseHandler'},
								{name: 'rewind',		kind: IconButton, small: false, backgroundOpacity: 'translucent', ontap: 'rewind'},
								{name: 'fsPlayPause',	kind: IconButton, small: false, backgroundOpacity: 'translucent', ontap: 'playPause'},
								{name: 'fastForward',	kind: IconButton, small: false, backgroundOpacity: 'translucent', ontap: 'fastForward'},
								{name: 'jumpForward',	kind: IconButton, small: false, backgroundOpacity: 'translucent', onholdpulse: 'onHoldPulseForwardHandler', ontap: 'onjumpForward', onrelease: 'onReleaseHandler'}
							]}
						]},
					...
					{name: 'rightPremiumPlaceHolder', kind: Control, classes: 'moon-video-player-premium-placeholder-right', components: [
						{name: 'moreButton', kind: IconButton, small: false, backgroundOpacity: 'translucent', ontap: 'moreButtonTapped'}
					]}
				]},

				{name: 'sliderContainer', kind: Control, classes: 'moon-video-player-slider-frame', components: [
					{name: 'slider', kind: MediaSlider, rtl: false, disabled: true, onSeekStart: 'sliderSeekStart', onSeek: 'sliderSeek', onSeekFinish: 'sliderSeekFinish',
	...
	spotlightUpHandler: function(sender, e) {
		if (this._shouldHandleUpDown) {
			...
			if (current.isDescendantOf(this.$.slider)) Spotlight.spot(this.$.fsPlayPause);
				...
				if (!this.$.videoInfoHeaderClient.getShowing()) this.showFSInfo();
	...
	*/
	mousemove: function(sender, e) {
		if (this.isOverlayShowing()) this.resetAutoTimeout();
		else if (this.shakeAndWake) this.showFSControls();
	},
	...
	*/
	onEnterSlider: function(sender, e) {
		if (this.hideButtonsOnSlider) this.$.controls.setShowing(false);
	},
	...
	*/
	timeUpdate: function(sender, e) {
		...
		if (!e && e.srcElement || e.currentTime == null) return;
	...
	*/
	backKeyHandler: function () {
		...
		if (visibleUp && visibleDown && MoonHistory.getCurrentObj() == this) {
