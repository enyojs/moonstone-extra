...
module.exports = kind(
	...
	*/
	spotlightDownHandler: function(sender, e) {
		if (this._shouldHandleUpDown) {
			...
			if (current.isDescendantOf(this.$.controls)) {
				this.$.slider.enterTapArea();
	...
	*/
	hideFSBottomControls: function() {
		...
		if (Spotlight.getCurrent().isDescendantOf(this.$.slider)) this.$.slider.leaveTapArea();
