...
module.exports = kind(
	...
	*/
	published: {
		...
		*/
		...
		/**
		* Sliders will increase or decrease as much as 5% in either direction
		* when left or right key is pressed in 5-Way mode.
		* However, some locale'd like to use specific value instead of percentage.
		* In the case, give value to this property when you instanciate moon.VideoPlayer.
		*
		* @type {Number}
		* @default null
		* @public
	...
	*/
	spotBlur: function () {
		Slider.prototype.spotBlur.apply(this, arguments);
	...
	* @private
	*/
	calculateKnobIncrement: function () {
		var increment = this.knobIncrement ? this.knobIncrement : '5%';

		if (typeof increment == 'number') return increment;
		else if (typeof increment == 'string' && increment.indexOf('%')) {
			return (this.max - this.min) * parseFloat(increment.split('%')[0]) / 100; 	
		} else return (this.max - this.min) / 20;	//default value is 5%
	},
	...
	*/
	spotLeft: function(sender, e) {
		if (this.selected) {
			...
			var v = this._value || this.getValue(),
				offset = this.calculateKnobIncrement();
			this._updateKnobPosition(v - offset)

			this.set('_value', v - offset);
			return true;
		}
	},
