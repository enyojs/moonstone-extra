...
module.exports = kind(
	...
	*/
	knobIncrementChanged: function () {
		var increment = this.knobIncrement;

		if (typeof increment == 'number') this._knobIncrement = increment;
		else if (typeof increment == 'string' && increment.indexOf('%')) {
			this._knobIncrement = (this.max - this.min) * parseFloat(increment.split('%')[0]) / 100; 	
		} else this._knobIncrement = (this.max - this.min) / 20;	//In case of undesirable input
	},
	...
	*/
	maxChanged: function() {
		Slider.prototype.maxChanged.apply(this, arguments);
		this.knobIncrementChanged();
	},
	...
	*/
	minChanged: function() {
		Slider.prototype.minChanged.apply(this, arguments);
		this.knobIncrementChanged();
	},
	...
	*/
	spotLeft: function(sender, e) {
		if (this.selected) {
			...
			this._updateKnobPosition(v - this._knobIncrement)
			this.set('_value', v - this._knobIncrement);
			return true;
		}
	},
