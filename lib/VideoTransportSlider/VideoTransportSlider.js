...
module.exports = kind(
	...
	*/
	knobIncrementChanged: function () {
		var increment = this.knobIncrement || _knobIncrement;

		if (typeof increment == 'number' && increment > 0) {
			this._knobIncrement = increment;
		} else if (typeof increment == 'string' && increment.indexOf('%') > 0) {
			this._knobIncrement = (this.max - this.min) * parseFloat(increment.split('%')[0]) / 100;	
		} else {
			// In case of enexpected string, object and etc
			increment = _knobIncrement;
			this._knobIncrement = (this.max - this.min) * parseFloat(increment.split('%')[0]) / 100; 
		}
	},
