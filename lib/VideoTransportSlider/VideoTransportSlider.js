...
module.exports = kind(
	...
	*/
	spotFocused: function (sender, e) {
		...
		if (!Spotlight.getPointerMode()) this._value = this.get('value');
		this.$.tapArea.waterfall('onenter');
		this.waterfall('onmousedown');
	},
	...
	*/
	spotBlur: function () {
		...
		this.$.tapArea.waterfall('onleave');
		this.waterfall('onmouseup');
	},
	...
	spotLeft: function(sender, e) {
		if (this.selected) {
			...
			var v = this._value || this.getValue(),
				offset = this.spotlightLRIncrement ? this.spotlightLRIncrement : (this.max - this.min) / 20;
